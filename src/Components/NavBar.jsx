  ...
  return (
    <div className="fixed top-48 left-2 rounded-2xl w-fit flex flex-col gap-2 z-20">
      ...
      <div className="group w-auto relative flex flex-col items-center">
        {/* First Menu */}
        {/* <ul className="flex flex-col items-center justify-center p-6 gap-8 w-14 bg-secoundary rounded-2xl opacity-0 group-hover:opacity-100 group-hover:scale-100 transition-all duration-1000 ease-in"> */}
        <ul className="flex flex-col items-center justify-center p-6 gap-8 w-14 bg-secoundary rounded-2xl opacity-30 group-hover:opacity-100 transition-all duration-900 ease-in">
          <li className="p-2 text-[25px] w-fit hover:bg-tertiary rounded-2xl text-white gap-2"><GoHome /></li>
          <li className="p-2 text-[25px] w-fit hover:bg-tertiary rounded-2xl text-white gap-2"><GrFavorite /></li>
          <li className="p-2 text-[25px] w-fit hover:bg-tertiary rounded-2xl text-white gap-2"><CiShoppingCart /></li>
          <li className="p-2 text-[25px] w-fit hover:bg-tertiary rounded-2xl text-white gap-2"><MdOutlinePermContactCalendar /></li>
        </ul>
        ...
        {menuItems.map((data, index) => (
          <NavLink key={index} to={data.to} className={({isActive}) => `p-2 text-[20px] w-full ${isActive? "bg-tertiary" : ""} hover:bg-tertiary rounded-2xl text-white flex items-center gap-2`}>{data.icon}{data.label}</NavLink>
        ))}
